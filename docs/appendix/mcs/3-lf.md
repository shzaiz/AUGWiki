...
表的第一行表示, 当命题 $P$ 为真时, 命题“非($P$)”为假. 第二行表示, 当 $P$
...
公式 (3.5) 在第一行中为真, 当 $A$、$B$ 和 $C$ 都为真时, 即
...
我们下面列出了一些等价公理, 用符号“$\leftrightarrow$”表示等价公式.
...
交换不同种类的量词（存在量词或全称量词）的顺序通常会改变命题的含义.
例如, 让我们回到最初的一个有歧义的陈述:
