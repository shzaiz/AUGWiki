...
- CPU是电脑的 “大脑”.
- 电脑运行过程中, CPU只能操作内存中的数据. 通常程序的数据会保存在内存中.
  内存存储速度非常快, 远远超过硬盘.
...
由于现在模拟的是把所有的电脑配件买回来 – 即配置一个架子, 那么除了
...
- Linux操作系统的前身Unix是开源操作系统, 其设计简单,
  保留了命令行的很多文化. 如Keep it simple and Stupid; Do one thing at a
  time, but do well等. 熟悉了命令行工具会让你的效率变高很多.
...
在启动界面中选择Try or install Ubuntu. 然后进行一些最基础的设置.
...
这些警告信息就像你的专属电力专家, 确保你不会因为小失误而让整个系统瘫痪.
